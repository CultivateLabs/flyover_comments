module FlyoverComments
  module CommentsHelper
    ...
    def flyover_comment_content(comment)
      ...
      elsif show_flagged_text?(comment)
    ...
    def show_flagged_text?(comment)
      try(:hide_flagged_comments?) && comment.flags.not_reviewed.exists? && !membership_can_view_flagged_comment?(comment)
    end

    def membership_can_view_flagged_comment?(comment)
    ...
    def flyover_comment_replies(comment, replies_expanded: false, children: comment.children)
      render "flyover_comments/replies/replies", comment: comment, replies_expanded: replies_expanded, children: children
    end
    ...
    def user_already_flagged_comment?(comment, user = _flyover_comments_current_user)
      FlyoverComments::Flag.where(comment: comment, flagger: user).exists?
    end
